...
int main(int argc, char *argv[]) {
    if (argc != 6){
        printf ("Usage: %s <delay> <delay2> <delay3> <delay4> <delay5>\n",argv[0]);
        exit(0);
    }
    pid_t pid[5];
    int i, j, n[5];
    n[0] = atoi(argv[1]), n[1] = atoi(argv[2]), n[2] = atoi(argv[3]), n[3] = atoi(argv[4]), n[4] = atoi(argv[5]); // n is a delay in microseconds inserted in parent and
                     // child iterations
    printf("\n Before forking.\n");
    for (i = 1; i < 5; i++) {
            pid[i] = fork();
